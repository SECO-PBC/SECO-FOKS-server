...
| 10 | `kvFileUploadInit` / `Chunk` (`file.go:325`) | writes | read role on md at init; **`Chunk` had no check of its own** | + ACL at creation (H3) for init, and in `assertUploading` for `Chunk` — that RPC names a file by ID with no directory in front of it, so a non-member who learned the ID could otherwise append to a tagged file mid-upload. The design does not rest on IDs being secret. |
