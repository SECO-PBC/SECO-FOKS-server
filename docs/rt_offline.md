...
- **The client needs a matching change, or the server's replay is rejected
  on arrival.** The send path validates that the acked seq is above the last
  locally-seen one — and a replay legitimately returns the *original*, older
  seq, so the check fires on exactly the case idempotency exists to serve.
  Confirmed by building D1 on a client without the outbox and watching the
  second send fail with `RTMsgOrderError` while the server behaved perfectly.
  The ordering check must therefore treat "this seq is already held locally by
  *my own* msgID" as a replay rather than a misordering, and return the
  original result without re-caching it. `finalizeAck` carries that tolerance
  in the full design (D2); a standalone D1 has to carry it too.
...
**Open: generalizing this to all teams.** `offline_verified_reads.md` (Q1)
recommends extending re-seal-at-drain beyond ad-hoc teams, on the grounds that
it solves staleness at its root: a message sealed at drain, after a fresh
verified load, is encrypted to the roster as of *send* time rather than compose
time, which removes the removed-member window above without ever refusing to
queue anything. Ratchet-based messengers arrive at the same arrangement by
necessity, since their key state advances per send and a queued message cannot
be pre-encrypted at all.

The blocker is this document's own invariant. The outbox stores the sealed
wrapper precisely so message plaintext never reaches the local DB, and a
message that is not sealed at compose time has to be held as something. The
shape that works is sealing to a local device key while queued and re-sealing
to the team's PTKs at drain — a change of key, not a loss of protection at
rest, and the same posture as a plaintext row inside an encrypted local
database.

Recorded here as an open design item, not a decision. Nothing in the built
system does this today: re-sealing remains ad-hoc-only, and the outbox remains
sealed-to-team. Whoever takes it up should amend D2's lifecycle and this
section together, since the invariant and the mechanism are the same choice
seen from two sides.
...
- **Cold-start offline removes an accidental liveness gate.** Before the
  ...
  could always read the local DB directly — but the gate was load-bearing by
  accident, and its removal should be a stated consequence of the design rather
