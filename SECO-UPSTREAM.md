...
| social signup implementation (server + protocol + schema) | **an upstream signal on the merged design doc** | Implements the design doc that merged as [#339](https://github.com/foks-proj/go-foks/pull/339) (Upstreamed below): `SocialInviteGuest @0xc6f4b985` on reg, `SocialInvite @0xee2c9d1c` on user, derivation variant `@0x8053b9737d1d8a37` (all three IDs are the spec's own), `social_invites`/`social_invite_msgs`, expiry sweep as a quota-server sub-sweep. Fork-local allocations to reconcile **by content** at proposal time, per the #376 precedent: `User.newInviteCode` is fork `@200` (reserved block; upstream shape is `@30`), status codes `@13001–@13003` (a fresh block, since the fork's last top-of-block pick collided at `@12008`), `foks_users` patch `p8`. Implementation choices the spec left open, to argue in the PR: `SocialInviteMsgPayload` as the canonical box payload (sealing needs a typed payload), transcript inline in `SocialInvite.list`, config knobs in jsonnet `Settings` rather than per-vhost `host_config`, no per-IP rate limit on the guest fetch (no such plumbing exists anywhere; the spec's 32-byte HMAC id argument covers it). |
