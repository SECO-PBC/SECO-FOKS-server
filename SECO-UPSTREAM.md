...
| channel mutation: rename, edit description, archive | **an answer on [#351](https://github.com/foks-proj/go-foks/issues/351)** — opened 2026-09-15, unanswered as of 09-22 | A channel's metadata is write-once: no RPC updates one, `channels.seqno` is stamped `1` at creation and never incremented, and nothing can be closed. Blocks the app's `✏️` rename control and **Archive** foot action (`blueprints/channels/CHANNELS.md` §5.1). Design in `docs/rt-channel-mutation.md`. **Fork half built and open as [#46](https://github.com/SECO-PBC/SECO-FOKS-server/pull/46)** (`feat/rt-channel-mutation`), not yet merged or tagged; it carries the spec and this row. Shape proposed to Max: `rtUpdateChannel` (CAS on `seqno`) plus `rtSetChannelArchived` as a **tombstone, not a purge** — an archived channel keeps every row, stays in the channel set so its name is reserved, and leaves the inbox. **Three numbering divergences to carry, all verified against `upstream/main` 2026-09-22, none of them obvious:** (1) upstream's next free `RTChannelMetadata` field is **@14**, not @13 — `noPush @13` merged with #365 at a different number from our @21; (2) upstream RPC **@12–@15 are claimed by our own open #370** (push holds), so the upstream proposal takes **@16/@17** while the fork ships @207/@208, and these move down if #370 is declined; (3) patch ids collide already — upstream p5 is no-push, fork p5 is `channel_acl`, so upstream's archive patch will not be our p9 and must be reconciled by CONTENT, as with no-push and push holds. Do not wait on Max to ship the fork half: the app has been blocked since 09-15 and the private-channel precedent is that RT features Max declines still ship at @200+. |
...
| `.github/workflows/ci.yml` package scoping | Ours. `integration-tests/` is now linted and, for `lib`, a test gate: the suite brings up postgres itself via testcontainers, so no `services:` block was needed -- only removing the exclusion. `cli` runs advisory (`continue-on-error`) because it flakes on signup/provisioning the same way it does upstream. Upstream runs plain `./...`. |
