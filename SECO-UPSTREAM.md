...
| no-push channels | [#365](https://github.com/foks-proj/go-foks/pull/365) | A creation-time per-channel flag that drops the channel from the `push_outbox` fan-out on send; the inbox-version bump is untouched, so online delivery is unchanged. Ours is fork PR #30; the upstream port differs in three ways worth remembering at merge time. It is numbered `noPush @13` (dense — upstream's `RTChannelMetadata` ends at @12; our @21 is the fork's sparse-numbering convention), it carries no private-channel vocabulary (`acl.go` and `channelForkCols` are fork-only, so the read-back rides plain `channelMetadataCols`), and its `MakeChannelOpts` is new upstream rather than an existing struct gaining a field. Opened before we had field numbers on notification volume — the PR says so rather than claiming a measured win. |
