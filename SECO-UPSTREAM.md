...
| libkv stale-cache retry fixes | **Stefan's go-ahead to open** — branch `upstream-pr/kv-stale-cache-retry` is pushed, off `upstream/main`, signed off, `integration-tests/lib` + `cli` green | Two defects in `cacheRaceLoop`, one PR, three commits (the third avoids an extra `KvCacheCheck` on every missing-file read that the noent fix would otherwise add). (1) `CacheAccess.clear()` dropped the party, so after any successful precondition-checked call a later `KVStaleCacheError` in the same loop cleared nothing and burned all 501 retries, then surfaced "cached values were stale" (the error `foks-gomobile/kv_helpers.go` `retryOnStaleCache` works around). (2) a cached tombstone answered `KVNoentError` with no cache check, so a file removed and re-written by someone else stayed "not found" for good. Tests: `integration-tests/lib/kv_stale_cache_test.go`. The fork-only commit on PR #35 (serve a cached noent offline) is **Local**: it patches the KV offline carve-out, which is not upstream. |
