...
| Change | PR | Notes |
|---|---|---|
| CLKR skips teams it cannot act on | [#373](https://github.com/foks-proj/go-foks/pull/373) | Opened 2026-09-21 from `upstream-pr/clkr-skip-non-admin` (off `e174ca6`). `CLKROneTeam.run` returns immediately when the client is below admin, but only after `loadTeam` has run `LoadTeamWithFQTeam` with `Refresh` + `MemberLoadFull`, refreshing the team and re-fetching **every member's user chain** — all discarded. Same antipattern already removed from `ListTeamRoster`. The sweep's own exploration loads teams at `MemberLoadNames`, whose roster carries roles, so the check is answerable with no round trip; such teams are now dropped before `loadTeam` and before the inter-team delay. Matters most to a client in many teams where it is an ordinary member: at the default 10s delay a sweep over N such teams cost N full loads and ~N*10s, and `BgJobMgr.tick` computes the next due time from the tick that *started* the run, so once a sweep passes the 17min interval it restarts on the next tick. **Also makes `CLKR.explore` index its exploration** (`tm.reindex`), which a review pass showed the pre-check needs to be sound: the sweep decides from the exploration's `TeamRecord` while `loadTeam` resolves through `TeamMinder.loadedTeams`, which CLKR left empty, so a second full exploration filled it — and since `explorePool` claims concurrently, a team reachable by two membership paths could be claimed by a different path each time, letting the sweep decline a team on one path's role that it would have loaded on the other's, differently run to run. Indexing also stops the sweep walking the graph twice. **Behaviour change, flagged in the PR body:** the pre-check sees only what the exploration saw, so a promotion landing mid-sweep is picked up on the following run; offered a `MemberLoadNone` refresh as the alternative if maxtaco would rather not have that window. Adds `Skipped()` alongside `Rekeys()` to make it testable. Not widened by this change, but worth knowing: a team claimed via a path where you are a reader is skipped even if another path would make you admin — `run()` already behaved that way, because `loadTeam` uses the same claimed record. Found while sizing whether a private channel could be backed by its own FOKS team — see `docs/kv-channel-acl.md` — but it is a pure upstream inefficiency with nothing fork-local. |
| RT offline cold-start bootstrap | [#361](https://github.com/foks-proj/go-foks/pull/361) | Serves verified local snapshots from the user, team and probe loaders, so a client starts and works with no network. Carries `verifiedAt` on `UserSigchainState` and `TeamChainState`, the offline-reads design doc, and a distinct "cannot verify server identity" signal on a repeat certificate failure against a host this device has verified before. Was stacked on [#359](https://github.com/foks-proj/go-foks/pull/359), which merged 2026-09-18; its diff should now read as its own change, and it may need a rebase onto `e174ca6`. Also carries a one-line test-harness fix: the fabricated merkle root epno seed was 1000, which the suite had outgrown, and the extra users these tests add pushed the real tree past it. |
