...
`upstream/main` has since moved to `b121099` (checked 2026-09-23).
...
| social signup implementation (server + protocol + schema) | **a human opening the prepared PR** — branch `upstream-pr/social-signup` is pushed, cut from `upstream/main` (`b121099`), DCO-signed, builds and vets clean; the Claude GitHub App has no access to `foks-proj/go-foks`, so open it at <https://github.com/foks-proj/go-foks/compare/main...SECO-PBC:upstream-pr/social-signup>, then move this row to Proposed with the number | Implements the design doc that merged as [#339](https://github.com/foks-proj/go-foks/pull/339) (Upstreamed below): `SocialInviteGuest @0xc6f4b985` on reg, `SocialInvite @0xee2c9d1c` on user, derivation variant `@0x8053b9737d1d8a37` (all three IDs are the spec's own), `social_invites`/`social_invite_msgs`, expiry sweep as a quota-server sub-sweep. Fork-local allocations, already reconciled **by content** on the prepared branch: `User.newInviteCode` is fork `@200` (reserved block) and `@30` on the branch; status codes `@13001–@13003` and `foks_users` patch `p8` carry over unchanged (both free upstream as of `b121099`; upstream `foks_users` p1–p7 are byte-identical to ours). Implementation choices the spec left open, to argue in the PR: `SocialInviteMsgPayload` as the canonical box payload (sealing needs a typed payload), transcript inline in `SocialInvite.list`, config knobs in jsonnet `Settings` rather than per-vhost `host_config`, no per-IP rate limit on the guest fetch (no such plumbing exists anywhere; the spec's 32-byte HMAC id argument covers it). |
