...
| large-file read-role checks (kv-store) | **nothing — branch pushed, PR not opened** | `upstream-pr/kv-large-file-read-checks`, commit `727c815`, cut from `upstream/main` with the DCO signoff. `loadLargeFileMetadata` and `getChunk` were the only read paths in `server/kv-store` that never compared the caller's role to the node's; `getDir`, `listDir`, `loadDirent` and `mLoadSmallFilesOrSymlinks` all do. Not a plaintext leak — the key box is sealed to the PTK at the file's read role — but existence, size and chunk layout went to any party member who knew the file ID, leaving the crypto as the only gate. One integration test per RPC; the `getChunk` one issues `kvGetEncryptedChunk` directly, because libkv loads metadata first and a test going through the client API passes with `getChunk`'s check removed. Removing either check fails exactly one test; the full `integration-tests/lib` suite is green. Prerequisite for the fork-only `docs/kv-channel-acl.md`, but stands on its own upstream. |
