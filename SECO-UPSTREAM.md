...
|---|---|---|
| kv-store: absent small-file `KvGetNode` panics the server | **nothing — slice off `feat/kv-channel-acl`** | Found 2026-09-21 by a masking probe in the channel-ACL tests: `loadSmallFileOrSymlink` returns a nil entry for a row that is not in the table (`mLoadSmallFilesOrSymlinks` appends its map lookup unconditionally), and `loadNode` dereferences the result — so `kvGetNode` for ANY absent small-file or symlink node ID is a remotely triggered panic, reachable by any authenticated caller, upstream and fork alike. The listing path already guards (`listDir` checks `f != nil`); the direct load was the one caller that did not. One-line fix plus the probe that found it; the fork carries it on `feat/kv-channel-acl`, and it should go upstream on its own branch off `upstream/main` like #371 did. |
