...
func (c RealTimeClient) ClientRTUpdateChannel(ctx context.Context, arg ClientRTUpdateChannelArg) (err error) {
	...
	var tmp rpc.DataWrap[Header, interface{}]
	err = c.Cli.Call2(ctx, rpc.NewMethodV2(RealTimeProtocolID, 8, "RealTime.clientRTUpdateChannel"), warg, &tmp, 0*time.Millisecond, realTimeErrorUnwrapperAdapter{h: c.ErrorUnwrapper})
	if err != nil {
		return
	}
	if c.CheckResHeader != nil {
...
func (c RealTimeClient) ClientRTSetChannelArchived(ctx context.Context, arg ClientRTSetChannelArchivedArg) (err error) {
	...
	var tmp rpc.DataWrap[Header, interface{}]
	err = c.Cli.Call2(ctx, rpc.NewMethodV2(RealTimeProtocolID, 9, "RealTime.clientRTSetChannelArchived"), warg, &tmp, 0*time.Millisecond, realTimeErrorUnwrapperAdapter{h: c.ErrorUnwrapper})
	if err != nil {
		return
	}
	if c.CheckResHeader != nil {
